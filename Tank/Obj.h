...
public:
	...
	virtual float		Update_Radian(POINT _focus);			// 타겟의 좌표로 각도를 반환합니다.
	
public: // 지속 회전 함수(회전 속도)
	virtual void		Rotation_Radian(int _speed);
						
public: //데이터 관리 함수
	virtual const RECT&					Get_Body(void) const { return m_Body; }
	virtual const INFO&					Get_Info(void) const { return m_Info; }
	virtual const OBJID::ID&			Get_Type(void) const { return m_Type; }
	...
	virtual void				Set_Type(OBJID::ID _type) { m_Type = _type; }
